          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginRight: '1rem' }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                if (newShow && (!post.comments || post.comments.length === 0) && commentsCount > 0) {
                  onLoadMoreComments(post._id, 1);
                }
              }}
              title="Comments"
              style={{ padding: '0.25rem' }}
      ...
      {/* Expanded Comments Section */}
      {
        showComments && (
          <div style={{
            ...
            {/* Lazy Load Trigger */}
            {showComments && post.comments.length === 0 && commentsCount > 0 && (
              <div style={{ textAlign: 'center', padding: '1rem' }}>
                {/* We trigger load on mount if empty. 
                       Better: use useEffect or just render Loader and call prop. 
                       Since we can't easily use useEffect conditionally inside this block without creating a sub-component or lifting up,
                       let's use a subtle trick: Render a component that calls it on mount? 
                       Or better, change the onClick handler for the button.
                   */}
                Loading comments...
              </div>
            )}
